...
$conn->query("CREATE TABLE IF NOT EXISTS `rollcalluser` (
...
// ساخت تاریخ کامل
$rollcallUserShamsiDate = $selectedYear . '/' . $selectedMonth . '/' . $selectedDay;
$RollcallUserDate = shamsiToGregorian($rollcallUserShamsiDate) ?? date('Y-m-d');
...
if ($classId && $selectedYear && $selectedMonth && $selectedDay) {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("SELECT * FROM `rollcalluser` WHERE ClassID = ? AND RollcallUserDate = ?");
        $stmt->bind_param('is', $classId, $RollcallUserDate);
        $stmt->execute();
        $rollcallUserResult = $stmt->get_result();
        
        while ($row = $rollcallUserResult->fetch_assoc()) {
            $RollcallUserData[$row['UserID']] = $row;
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_rollcalluser'])) {
    ...
    // ساخت تاریخ کامل
    $rollcallUserShamsiDate = $selectedYear . '/' . $selectedMonth . '/' . $selectedDay;
    $rollcallUserDate = shamsiToGregorian($rollcallUserShamsiDate);
    if (!$rollcallUserDate) {
    ...
    if (!$errors) {
        ...
        try {
            foreach ($_POST['rollcalluser'] as $UserId => $status) {
            ...
            header('Location: rollcalluser.php?success=1&class_id=' . $classId . '&year=' . $selectedYear . '&month=' . $selectedMonth . '&day=' . $selectedDay);
            exit();
    ...
    <div class="content-box">
        ...
        <form method="post" id="rollcalluser-form">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive rollcallUser-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="badge bg-secondary"><?php echo htmlspecialchars($User['UserSysCode']); ?></span>
                                    </td>
                                    <td class="text-center">
                                        <span class="text-muted"><?php echo htmlspecialchars($User['UserMelli']); ?></span>
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td>
                                        <input type="text" name="notes[<?php echo $User['UserID']; ?>]" 
                                               class="form-control form-control-sm" 
                                               placeholder="توضیحات (اختیاری)"
                                               value="<?php echo htmlspecialchars($rollcallUser['Notes'] ?? ''); ?>">
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
...
document.getElementById('rollcalluser-form')?.addEventListener('submit', function(e) {
