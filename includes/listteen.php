...
if ($q !== '') {
    ...
    $where = "WHERE (TeenMelli LIKE '%$q_esc%' OR TeenSysCode LIKE '%$q_esc%' OR TeenName LIKE '%$q_esc%' OR TeenFamily LIKE '%$q_esc%')";
}

$total = 0;
$rsCount = $conn->query("SELECT COUNT(*) c FROM Teen $where");
if ($rsCount) { $total = (int)$rsCount->fetch_assoc()['c']; }

$rs = $conn->query("SELECT * FROM Teen $where ORDER BY TeenID DESC LIMIT $perPage OFFSET $offset");
?>
<!DOCTYPE html>
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenSysCode']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenMelli']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenName']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenFamily']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenFather']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenMobile1']); ?></td>
                        <td><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenCity'] ?? '-'); ?></td>
                        <td>
                            <?php 
...
requireAccess(basename(__FILE__));
$statusClass = '';
                            if ($row['TeenStatus'] === 'عادی') {
                                $statusClass = 'text-success';
                            } elseif ($row['TeenStatus'] === 'فعال') {
                                $statusClass = 'text-warning';
                            } elseif ($row['TeenStatus'] === 'تعلیق') {
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($row['TeenStatus']); ?>
                            </span>
                        </td>
                        <td>
                            <a class="btn btn-sm btn-primary" href="editteen.php?TeenID=<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $row['TeenID']; ?>" title="ویرایش">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <a class="btn btn-sm btn-info" href="viewteen.php?TeenID=<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $row['TeenID']; ?>" title="مشاهده">
                                <i class="bi bi-eye"></i>
                            </a>
                        </td>
                    </tr>
...
requireAccess(basename(__FILE__));
        ...
        <div class="mt-4 text-center">
            <a href="regteen.php" class="btn btn-primary btn-lg">
