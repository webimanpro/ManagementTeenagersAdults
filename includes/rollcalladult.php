...
// ایجاد جدول حضور و غیاب بزرگسالان اگر وجود ندارد
$conn->query("CREATE TABLE IF NOT EXISTS `rollcalladult` (
  `RollcalladultID` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`RollcalladultID`),
  UNIQUE KEY `unique_rollcalladult` (`ClassID`,`AdultID`,`RollcalladultDate`)
...
// ساخت تاریخ کامل
$attendanceShamsiDate = $selectedYear . '/' . $selectedMonth . '/' . $selectedDay;
$RollcalladultDate = shamsiToGregorian($attendanceShamsiDate) ?? date('Y-m-d');
...
if ($classId && $selectedYear && $selectedMonth && $selectedDay) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (!empty($adultIds)) {
            $placeholders = str_repeat('?,', count($adultIds) - 1) . '?';
            $stmt = $conn->prepare("SELECT * FROM `adult` WHERE AdultID IN ($placeholders) ORDER BY AdultFamily, AdultName");
            $stmt->bind_param(str_repeat('i', count($adultIds)), ...$adultIds);
            $stmt->execute();
            $adults = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        }
        
        // دریافت اطلاعات حضور و غیاب برای تاریخ انتخاب شده
        $stmt = $conn->prepare("SELECT * FROM `rollcalladult` WHERE ClassID = ? AND RollcalladultDate = ?");
        $stmt->bind_param('is', $classId, $RollcalladultDate);
        $stmt->execute();
        $attendanceResult = $stmt->get_result();
        
        while ($row = $attendanceResult->fetch_assoc()) {
            $attendanceData[$row['AdultID']] = $row;
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_attendance'])) {
    ...
    // ساخت تاریخ کامل
    $attendanceShamsiDate = $selectedYear . '/' . $selectedMonth . '/' . $selectedDay;
    $RollcalladultDate = shamsiToGregorian($attendanceShamsiDate);
    if (!$RollcalladultDate) {
    ...
    if (!$errors) {
        ...
        $dayName = $dayNames[date('l', strtotime($RollcalladultDate))];
        
        $conn->begin_transaction();
        
        try {
            foreach ($_POST['attendance'] as $adultId => $status) {
                $notes = $_POST['notes'][$adultId] ?? '';
                
                // بررسی آیا رکورد قبلی وجود دارد
                $checkStmt = $conn->prepare("SELECT RollcalladultID FROM `rollcalladult` WHERE ClassID = ? AND AdultID = ? AND RollcalladultDate = ?");
                $checkStmt->bind_param('iis', $classId, $adultId, $RollcalladultDate);
                $checkStmt->execute();
                ...
                if ($exists) {
                    ...
                    $stmt = $conn->prepare("UPDATE `rollcalladult` SET Status = ?, Notes = ? WHERE ClassID = ? AND AdultID = ? AND RollcalladultDate = ?");
                    $stmt->bind_param('ssiis', $status, $notes, $classId, $adultId, $RollcalladultDate);
                } else {
                    // درج رکورد جدید
                    $stmt = $conn->prepare("INSERT INTO `rollcalladult` (ClassID, AdultID, RollcalladultDate, RollcalladultDay, Status, Notes) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param('iissss', $classId, $adultId, $RollcalladultDate, $dayName, $status, $notes);
                }
            ...
            header('Location: presentadult.php?success=1');
            exit();
...
// دریافت لیست دوره‌ها (فقط دوره‌های مربوط به بزرگسالان)
$classes = $conn->query("SELECT ClassID, ClassName FROM `Class` WHERE UserType = 'adult' OR UserType = 'both' OR CalssUsers LIKE '%\"type\":\"adult\"%' ORDER BY ClassName");
...
<head>
    ...
    <title>حضور و غیاب بزرگسالان</title>
...
<div class="container main-container">
    <div class="content-box">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <h2 class="mb-0">حضور و غیاب بزرگسالان</h2>
        </div>
        </div>
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $classId == $class['ClassID'] ? 'selected' : ''; ?>>
                                    <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $selectedYear == $year ? 'selected' : ''; ?>>
                                    <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $selectedMonth == $key ? 'selected' : ''; ?>>
                                    <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
                ...
                <div class="row mt-3">
                    <div class="col-6">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-search me-2"></i>نمایش لیست بزرگسالان
                         </button>
                <!-- دکمه مشاهده گزارش نوجوانان - فقط برای مدیران -->
                <?php if (isAdmin()): ?>
                        <a href="reportadult.php?class_id=<?php echo $classId; ?>&year=<?php echo $selectedYear; ?>&month=<?php echo $selectedMonth; ?>" 
                           class="btn btn-info col-6" 
                           target="_blank">
                            <i class="bi bi-bar-chart me-2"></i>مشاهده گزارش بزرگسالان
                        </a>
                    <?php endif; ?>
					</div>
                </div>
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
if ($classId && $selectedYear && $selectedMonth && $selectedDay && !empty($adults)): ?>
        <!-- فرم حضور و غیاب -->
...
echo $selectedDay; ?>">
            ...
            <div class="card">
                <div class="card-header bg-primary text-white">
                    ...
                    لیست بزرگسالان دوره: <strong><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($classData['ClassName']); ?></strong>
                    - تاریخ: <strong><?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $selectedYear . '/' . $selectedMonth . '/' . $selectedDay; ?></strong>
                </div>
...
foreach ($adults as $index => $adult): 
                                    $attendance = $attendanceData[$adult['AdultID']] ?? [];
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($adult['AdultName'] . ' ' . $adult['AdultFamily']); ?></strong>
                                    </td>
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($adult['AdultSysCode']); ?></span>
                                    </td>
                                    <td class="text-center">
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($adult['AdultMelli']); ?></span>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <input type="radio" class="btn-check" name="attendance[<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>]" 
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>" value="present" 
                                                   <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $currentStatus === 'present' ? 'checked' : ''; ?>>
                                            <label class="btn btn-outline-success" for="present_<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>">
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>" value="absent" 
                                                   <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $currentStatus === 'absent' ? 'checked' : ''; ?>>
                                            <label class="btn btn-outline-danger" for="absent_<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>">
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>" value="excused" 
                                                   <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $currentStatus === 'excused' ? 'checked' : ''; ?>>
                                            <label class="btn btn-outline-warning" for="excused_<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>">
                                    ...
                                    <td>
                                        <input type="text" name="notes[<?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo $adult['AdultID']; ?>]" 
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
echo htmlspecialchars($attendance['Notes'] ?? ''); ?>">
                                    </td>
                                </tr>
                                <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
...
endforeach; ?>
                    ...
                    <div class="mt-4 text-center">
                        ...
                        <a href="presentadult.php" class="btn btn-outline-secondary btn-lg">
...
require_once __DIR__ . '/check_access.php';
requireAccess(basename(__FILE__));
elseif ($classId && $selectedYear && $selectedMonth && $selectedDay && empty($adults)): ?>
            <div class="alert alert-warning text-center">
                <i class="bi bi-exclamation-triangle me-2"></i>
                هیچ بزرگسالی در این دوره ثبت نام نکرده است.
            </div>
        <?php 
// ط¨ط±ط±ط³غŒ ط¯ط³طھط±ط³غŒ ع©ط§ط±ط¨ط±
require_once __DIR__ . '/check_access.php';
