...
// Load current user from existing `users` table
$row = null;
if ($sessionUserId > 0) {
    if ($stmt = $conn->prepare("SELECT id, username, full_name, email, phone, role FROM users WHERE id=? LIMIT 1")) {
...
if (!$row && $sessionUsername) {
    if ($stmt = $conn->prepare("SELECT id, username, full_name, email, phone, role FROM users WHERE username=? LIMIT 1")) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_new_user']) && $currentUserRole === 'admin') {
    ...
    if (!$errors) {
        ...
        $insertStmt = $conn->prepare("INSERT INTO users (username, password, full_name, email, phone, role) VALUES (?, ?, ?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_user']) && $currentUserRole === 'admin') {
    ...
    if (!$errors && $editUserId > 0) {
        if (!empty($editPassword)) {
            ...
            $updateStmt = $conn->prepare("UPDATE users SET username=?, password=?, full_name=?, email=?, phone=?, role=? WHERE id=?");
            $updateStmt->bind_param('ssssssi', $editUsername, $hashedPassword, $editFullName, $editEmail, $editPhone, $editRole, $editUserId);
        } else {
            // بدون تغییر رمز عبور
            $updateStmt = $conn->prepare("UPDATE users SET username=?, full_name=?, email=?, phone=?, role=? WHERE id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['add_new_user'])) {
    ...
    if (!$errors) {
        ...
        if ($Password !== '') {
            ...
            if ($useId) {
                $sql = "UPDATE users SET username=?, password=?, phone=?, email=?, role=?, full_name=? WHERE id=?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('ssssssi', $Username, $hash, $Mobile, $Email, $Role, $FullName, $row['id']);
            } else {
                $sql = "UPDATE users SET password=?, phone=?, email=?, role=?, full_name=? WHERE username=?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('ssssss', $hash, $Mobile, $Email, $Role, $FullName, $sessionUsername);
            }
        } else {
            if ($useId) {
                $sql = "UPDATE users SET username=?, phone=?, email=?, role=?, full_name=? WHERE id=?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('sssssi', $Username, $Mobile, $Email, $Role, $FullName, $row['id']);
            } else {
                $sql = "UPDATE users SET phone=?, email=?, role=?, full_name=? WHERE username=?";
                $stmt = $conn->prepare($sql);
        ...
        if ($stmt && $stmt->execute()) {
            ...
            // Reload row
            if ($useId) {
                $stmt2 = $conn->prepare("SELECT id, username, full_name, email, phone, role FROM users WHERE id=?");
                $stmt2->bind_param('i', $row['id']);
            } else {
                $stmt2 = $conn->prepare("SELECT id, username, full_name, email, phone, role FROM users WHERE username=?");
...
<div class="container" style="margin-top:100px; margin-bottom:40px; text-align:right;">
    ...
    $allUsersForModals = $conn->query("SELECT * FROM users ORDER BY created_at DESC");
    while ($user = $allUsersForModals->fetch_assoc()):
    ?>
